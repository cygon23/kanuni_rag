...
def _build_app(db_pool: asyncpg.Pool, tmp_path: Path) -> FastAPI:
...
async def test_reuploading_the_same_file_is_reported_as_skipped(
    ...
    client = TestClient(_build_app(db_pool, tmp_path), raise_server_exceptions=False)
...
async def test_different_files_produce_separate_documents(
    db_pool: asyncpg.Pool, tmp_path: Path
) -> None:
